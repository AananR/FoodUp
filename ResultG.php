...
<body>
  <section class="menu cid-qOyNx7OUcv" once="menu" id="menu1-56">
...
<section class="engine"></section><section class="header1 cid-qOB2WzOksg mbr-parallax-background" id="header1-57">
...
$result = pg_query($conn, "SELECT RESTAURANT.NAME, LOCATIONS.PHONE_NUMBER, RESTAURANT.TYPE
FROM RESTAURANT FULL OUTER JOIN LOCATIONS ON RESTAURANT.RESTAURANTID = LOCATIONS.RESTAURANTID
FULL OUTER JOIN RATING ON RATING.RESTAURANTID = RESTAURANT.RESTAURANTID
WHERE RATING.DATE = (SELECT MAX(RATING.DATE) FROM RATING 
        WHERE RATING.DATE < '2015/01/01') " );
while ($row = pg_fetch_row($result)) {
  echo "Name: $row[0],   PHONE_NUMBER: $row[1], TYPE: $row[2] ";
  echo "<br />\n";
}
